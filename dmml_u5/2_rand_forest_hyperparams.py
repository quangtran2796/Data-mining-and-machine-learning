...
def evaluate_n_estimators(X: np.ndarray, y: np.ndarray, n: int) -> (float, float):
    ...
    return np.mean(scores_rfc["train_score"]), np.mean(scores_rfc["test_score"])
...
def evaluate_depth(X: np.ndarray, y: np.ndarray, depth: int) -> (float, float):
    ...
    mean train and test score."""
    scores_rfc = cross_validate(
         estimator=RandomForestClassifier(n_estimators =20, max_depth = depth),
         X=X,
         y=y,
         scoring="accuracy",
         cv=10,
...
def evaluate_features(X: np.ndarray, y: np.ndarray, n_features: int) -> (float, float):
    ...
    mean train and test score."""
    #Thang nay chi khoang tu 11 den 20 features la tot thoi.
    scores_rfc = cross_validate(
         estimator=RandomForestClassifier(n_estimators =20,max_features = n_features),
         X=X,
         y=y,
         scoring="accuracy",
         cv=10,
