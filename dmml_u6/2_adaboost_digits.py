...
def get_acc_for_estimators(base_estimator: DecisionTreeClassifier, X_train: np.ndarray,
    ...
    """Fits an AdaBoostClassifier on the given training data and returns the training and test accuracy for all
     iterations."""
    #Bai nay ko tu code ma copy code -> lan sau coi ky lai.
    ...
    accuracy_train = [accuracy_score(y_pred, y_train) for y_pred in model.staged_predict(X_train)]
    accuracy_test = [accuracy_score(y_pred, y_test) for y_pred in model.staged_predict(X_test)]
