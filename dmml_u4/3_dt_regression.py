...
def get_dt_prediction(X_train: np.ndarray, y_train: np.ndarray, max_depth: int) -> np.ndarray:
    ...
    #Tao ra 1 cai cay voi mot chieu sau dinh san.
...
def plot_regression_models(X_cont: np.ndarray, y_cont: np.ndarray, y_1: np.ndarray, y_2: np.ndarray) -> None:
    ...
    #Day la ham cua solution -> chi mang tinh chat ve voi -> khi co thoi gian thi nen xem lai
...
def main():
    ...
    # Doi voi cach viet nay thi no se tra ve 2 mang 2 va 5, sau do no se tu nhet vao y_1 va y_2
