    ...
    """Create polynomial transformation of degree d:
     Maps each datapoint x_i, x_(i+1), ..., x_k to its polynomial:
      [x_i, x_(i+1), ..., x_k] -> [[x_i^0,     ..., x_i^d],
                                   ...
                                   [x_k^0,     ..., x_k^d]]."""
    x_poly = []
    #Ham reshape nay se thay doi kich thuoc mang ban dau ve shape moi mong muon.Trong th = -1 thi se la 1 vector.
    #Trong phuong phap nay se Map ve ma tran nhu tren, sau do nhet bo tham so vao va tinh nhu linear binh thuong.
    for xi in x.reshape(-1):
        xi_poly = [xi ** d for d in range(degree + 1)]
        x_poly.append(xi_poly)
    return np.array(xi_poly)    
